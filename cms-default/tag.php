 ...
 *   $currentPage  – int
 *   $totalPages   – int
 *   $tag          – string, der aktive Tag-Name
 *
...
$tagName     = htmlspecialchars($tag ?? ($_GET['tag'] ?? ''), ENT_QUOTES, 'UTF-8');
$tagRaw      = $tag ?? ($_GET['tag'] ?? '');
...
<div class="archive-header" style="background:var(--surface-tint);border-bottom:1px solid var(--rule);padding:2.5rem 0 2rem;">
    <div class="container" style="max-width:var(--max);margin:0 auto;padding:0 1.5rem;">
        <nav class="breadcrumb" aria-label="Breadcrumb" style="font-size:.8rem;color:var(--ink-muted);margin-bottom:.75rem;">
            ...
            <span style="margin:0 .4rem;">›</span>
            <span style="color:var(--ink);">Tag: <?php echo $tagName; ?></span>
        </nav>
        <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.5rem;">
            <span style="font-size:.72rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--accent);">Tag-Archiv</span>
        </div>
