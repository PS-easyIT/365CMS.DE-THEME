...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_comment'])) {
    ...
    if (!\CMS\Security::instance()->verifyToken($_POST['csrf_token'] ?? '', 'comment_post_' . ($post['id'] ?? 0))) {
        ...
        $name = htmlspecialchars(trim((string) ($_POST['comment_name'] ?? '')), ENT_QUOTES);
        $email = filter_var(trim((string) ($_POST['comment_email'] ?? '')), FILTER_VALIDATE_EMAIL);
        $text = htmlspecialchars(trim((string) ($_POST['comment_text'] ?? '')), ENT_QUOTES);
        ...
        if ($name === '' || !$email || $text === '') {
            ...
            try {
                $db->execute(
                    ...
                    [(int) ($post['id'] ?? 0), $name, (string) $email, (string) ($_SERVER['REMOTE_ADDR'] ?? ''), $text]
                );
                header('Location: ' . $siteUrl . '/blog/' . rawurlencode((string) ($post['slug'] ?? '')) . '?commented=1#comments');
                exit;
            } catch (\Throwable) {
