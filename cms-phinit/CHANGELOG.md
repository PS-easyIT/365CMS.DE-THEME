...
| 🔴 fix | EditorJS / Bilder | `assets/css/rich-content.css` setzt Breite (Normal = natürliche Breite, Breit = bis 1000 px, Strecken = 100 %), Ausrichtung (links/mittig/rechts), Skalierung (`contain`, `cover`, `fill`, `none`, `scale-down`) und Maximalhöhe aus sanitizer-festen Renderer-Klassen. Damit bleiben alle Admin-Einstellungen auch nach Purifier-/Inline-Style-Bereinigung im Public-Frontend sichtbar. |
| 🟢 feat | EditorJS / Einzelbild-Quelle | Einzelbilder können jetzt eine sichere Quellwebsite angeben. `assets/css/rich-content.css` gestaltet die Public-Zeile **„QUELLE ↗“** unter der optionalen Bildunterschrift als dezenten externen Link. |
| 🔴 fix | Public-Content / Bilder | `includes/theme-content-helpers.php` ersetzt den früheren HTML-escapenden Notfallpfad durch einen DOM-basierten Allowlist-Sanitizer. Dadurch bleiben sichere EditorJS-`figure`/`img`/`figcaption`-/Quellenlink-Markups sichtbar, während unzulässige Tags, Event-Attribute und gefährliche URL-Schemata entfernt werden. |
