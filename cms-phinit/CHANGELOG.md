...
| 🎨 style | Startseite / Featured-Banner | `assets/css/homepage-blog.css` entschärft den Featured-Banner zu einer dezenteren Editorial-Card mit ruhigerem Beitragsbild, sanfterem Farbverlauf und einer harten Titelobergrenze von `28px`. |
| 🎨 style | Startseite / Abschnittsbänder | `assets/css/content-cards.css` verstärkt die Abschnittsbänder für Artikel, Themenbereiche und Archiv mit dunklerem Verlauf, linker Akzentkante und klarerer Trennlinie. |
| 🎨 style | Startseite / Featured-Banner | `partials/home-featured-banner.php` und `assets/css/homepage-blog.css` zeigen das Beitragsbild wieder als passende linke Thumbnail-Spalte neben Titel/Textauszug, begrenzen den Banner auf maximal `250px`, setzen die Überschrift auf maximal `28px` und positionieren den Weiterlesen-Button unten rechts in der Featured-Card. |
| 🔴 fix | Startseite / Grid & Featured-Banner | `assets/css/content-cards.css` stellt die fehlenden `.post-card`- und Grid-Card-Regeln wieder her; `partials/home-featured-banner.php` rendert den Featured-Banner bewusst ohne Beitragsbild, sodass nur der dezente Farbverlauf sichtbar bleibt. |
| 🎨 style | Typografie / Startseite | `admin/customizer-schema.php`, `theme.json`, `includes/theme-assets-trait.php`, `assets/js/customizer-admin.js`, `style.css`, `assets/css/content-cards.css` und `assets/css/homepage-blog.css` trennen Body- und Display-Schrift mit Inter/Space Grotesk, setzen die Fließtext-Zeilenhöhe auf blogtaugliche `1.75`, machen sie im Customizer anpassbar und ersetzen schwere Section-Balken sowie den Featured-Banner durch dezente Editorial-Styles. |
...
| 🎨 style | Header / Hauptnavigation | `assets/css/header-navigation.css` hebt den aktiven Hauptnavigationspunkt zusätzlich mit orangem Unterstrich und leicht hellerem Text hervor. |
...
| 🟢 feat | Startseite / Featured-Banner | `admin/customizer-schema.php`, `includes/theme-home-helpers.php`, `index.php`, `partials/home-featured-banner.php` und `assets/js/homepage-widgets.js` erlauben bis zu drei Featured-Banner-Beiträge und wechseln beim Neuladen automatisch zum nächsten hinterlegten Beitrag. |
| 🟢 feat | Startseite / Sidebar Widgets | `partials/home-article-list.php`, `includes/theme-home-helpers.php`, `admin/customizer-schema.php` und `assets/css/homepage-blog.css` ergänzen Schnelllinks-, Kontakt- und Newsletter/Updates-Widgets inklusive Customizer-Steuerung und Sortierung. |
| 🟢 feat | Startseite / Microsoft 365 Widget | `partials/home-article-list.php`, `includes/theme-home-helpers.php`, `admin/customizer-schema.php`, `theme.json` und `assets/css/homepage-blog.css` ergänzen ein separates Microsoft-365-Schnelllinks-Widget mit 2×3-Linkraster und gekürztem „Admin Links“-Label. |
| 🟢 feat | Startseite / Site-Branding | `partials/home-article-list.php`, `includes/theme-home-helpers.php`, `admin/customizer-schema.php`, `theme.json` und `assets/css/homepage-blog.css` ergänzen ein optionales MVP-Badge rechts neben dem Sidebar-Logo. |
| 🔴 fix | Theme Customizer / Sidebar | `partials/home-article-list.php` und `assets/js/customizer-admin.js` berücksichtigen `m365links` auch im Frontend-Order-Mapping und synchronisieren die Widget-Reihenfolge direkt vor dem Speichern. |
| 🎨 style | Startseite / Sidebar Widgets | `assets/css/homepage-blog.css` vereinheitlicht Widget-Header, korrigiert die Header-Spezifität des Artikel-Karussells und entfernt den Abstand zwischen Karussell-Header und Vorschaubild. |
| 🎨 style | Startseite / Mobile | `assets/css/homepage-blog.css` zeigt die Startseiten-Sidebar auf Mobile nach der Artikelliste vollbreit an, stapelt Artikelkarten mit Bild oben und erhöht wichtige Touch-Targets auf mindestens 44px. |
| 🎨 style | Startseite / Buttons | `assets/css/homepage-blog.css` und `assets/css/content-cards.css` stellen „Weiter lesen“-Links als Ghost-Buttons mit Akzent-Hover dar. |
| 🎨 style | Startseite / Schnelllinks-Widget | `assets/css/homepage-blog.css` zeigt Schnelllinks im Sidebar-Widget konsequent als zweispaltiges Button-Raster mit zwei Buttons pro Reihe. |
| 🎨 style | Startseite / Projekt-Widget | `assets/css/homepage-blog.css` halbiert die Höhe der Projektkarten in der Sidebar für Bild- und Initialen-Fallback-Varianten. |
