...
- ✅ **SEC-09 / Core-Member-URL-Härtung**: Der Member-Save-Pfad im Core validiert `website`, `social` und `avatar` jetzt serverseitig vor dem Persistieren; `MemberService` und `MemberController` normalisieren öffentliche URLs/Medienpfade konsistenter, öffentliche Autorprofile verwerfen ungültige URL-Felder bereits beim Datenaufbau, und auch Core-Dashboard-/Favoritenlisten reichen gespeicherte Links nur noch bereinigt an die Views weiter.
- ✅ **SEC-10 / Core-Member-Widget-Payloads**: Core-Dashboard-Views validieren jetzt auch Onboarding-CTA-Links, Plugin-Widget-Links und designnahe Farbwerte defensiv, bevor diese in `href`-Attribute oder Inline-CSS-Variablen gelangen. Dadurch hängen Member-Settings und Plugin-Widget-Payloads enger an einem kleinen Allowlist-Vertrag statt an rohen View-Annahmen.
- ✅ **SEC-11 / Zentrale Widget-Normalisierung**: `MemberController::getPluginWidgets()` und `getCustomWidgets()` normalisieren Widget-Titel, Icons, Beschreibungen, Badges, Links, Stats und Farbwerte jetzt zusätzlich zentral vor der View-Weitergabe. Plugin-Filter und Registry-Payloads treffen damit zuerst auf einen kleinen Controller-Vertrag, statt ausschließlich auf Escape-Logik in einzelnen Templates zu vertrauen.
- ✅ **SEC-12 / Widget-Herkunft gehärtet**: `PluginDashboardRegistry` und das Admin-Modul für Member-Widgets normalisieren Plugin-Metadaten jetzt bereits an der Quelle. Plugin-Konfigs, DB-Overrides und Admin-Preview-Kacheln reichen damit Titel, Icons, Farben, Badges und Admin-Links nicht mehr roh weiter, sondern folgen schon vor Controller/View einem kleinen Hex-/HTTP(S)-/Plaintext-Vertrag.
- ✅ **SEC-13 / Theme-Member-Dashboard-Settings synchronisiert**: Das `cms-phinit`-Dashboard nutzt die bereits gehärteten Core-Settings `member_dashboard_logo`, `member_dashboard_greeting` und `member_dashboard_welcome_text` jetzt als sichere Fallbacks. Das Theme rendert das Dashboard-Logo nur noch über `phinit_normalize_public_media_url()`, respektiert den Core-`show_welcome`-Schalter als Default und schließt damit die bisherige Lücke zwischen Admin-Konfiguration und Theme-Override.
- ✅ **SEC-14 / Member-Listen mit defensiven Datums- und Linkpfaden**: `member/dashboard.php` und `member/comments.php` formatieren Kommentar-/Favoriten-Daten jetzt fail-closed statt implizit über `strtotime(... ?? 'now')`, casten reine Zähler/Paginierungswerte explizit zu Integern und erzeugen Post-Links in den Member-Listen über den kanonischen Post-URL-Helfer plus Public-URL-Allowlist statt über manuelle `siteUrl/slug`-Verkettung.
- ✅ **SEC-15 / Feed-Digest-Ausgabe defensiv typisiert**: `member/feeds.php` formatiert Versand- und Abrufzeitstempel jetzt fail-closed, statt ungültige Werte implizit über `strtotime()` in scheinbar valide Zeiten umzudeuten, und gibt Zähler-/Badge-Werte im Abo-/Auswahlbereich explizit als Integer aus.
- ✅ **SEC-16 / Frontend-Datumspfade fail-closed**: `404.php`, `page-wide.php`, `page.php`, `partials/post-comments.php`, `partials/post-tech-card.php` und `partials/search-result-row.php` behandeln Zeitstempel jetzt defensiv. Ungültige Datumswerte erzeugen keine scheinbar aktuellen oder still verfälschten Anzeigen mehr; zusätzlich laufen Such- und 404-Ziel-URLs bzw. Medienpfade nicht mehr ungeprüft über rohe Fallback-Konstruktionen.
- ✅ **SEC-17 / Restliche Member-Datumsanzeigen vereinheitlicht**: `member/profile.php`, `member/security.php`, `member/forum.php` und `member/posts.php` rendern Nutzer-, Sitzungs-, Thread- und Beitragszeitstempel jetzt ebenfalls fail-closed statt implizit mit `strtotime(... 'now')`-ähnlichem Verhalten. Nebenbei wurden einzelne Statistikwerte explizit typisiert und Forum-/Post-Links defensiv über vorhandene Public-URL-Pfade abgesichert.
- ✅ **SEC-18 / Landing- und Autoren-Medienpfade nachgehärtet**: `page-landing.php` führt CTA- und Feature-Links jetzt fail-closed über die Public-URL-Allowlist und rendert Hero-Bilder nur noch über `phinit_normalize_public_media_url()`. Zusätzlich normalisieren `partials/page-header-block.php`, `author.php` und `authors.php` Seiten-Hero-, Avatar- und Beitragsbilder über denselben Public-Media-Vertrag statt rohe Meta-/DB-Werte direkt in `img src` zu schreiben.
- ✅ **SEC-19 / Archiv- und Sidebar-Links über Helper-Vertrag gezogen**: `partials/page-image-archive.php` normalisiert Download-/Bildziele sowie verknüpfte Artikel-Links jetzt über die Public-URL-/Media-Helper, während `partials/post-sidebar-social.php` und `partials/page-sidebar-nav.php` rohe Social-/Navigationsziele fail-closed verwerfen, statt ungeprüfte Payloads direkt als `href` auszugeben.
